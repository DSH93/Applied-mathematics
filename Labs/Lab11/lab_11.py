...
def subimage_iterator(img, width, height, ax, step_size, callback):
    ...
    greenFrame = np.zeros(img.shape)
    greenFrame[0, :width, 1] = 1
    greenFrame[:height, 0, 1] = 1
    greenFrame[height, :width, 1] = 1
    greenFrame[:height, width, 1] = 1

    for i in range(0, img.shape[0], step_size):
        for j in range(0, img.shape[1], step_size):
            ax.imshow(img + np.roll(greenFrame, (i, j), axis=(0, 1)))
            searchingMat = np.roll(img, (-i, -j), axis=(0, 1))
            yield searchingMat[:height, : width, :], i, j
            ax.clear()
...
def highlight_circle(results, img, height, width, figure, ax, fig, nth=4):
    ...
    ax.imshow(img + np.roll(greenFrame, (j, i), axis=(0, 1)))
